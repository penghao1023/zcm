...
function fieldRightClick(channel, field) {
  ...
  var newChart = $("<a />", { tabindex: -1, href: "javascript:void(0);" })
    .text("New Graph")
  ...
  for (var chart in chartViewers) {
    console.log(chart);
    var item = $("<a />", { tabindex: -1, href: "javascript:void(0);" })
  ...
  $("#chart-menu li").on("click", function() {
    var idx = $(this)
      .find("a")
      .attr("data-viewer-idx");
    if (idx == -1) {
      var chart = new chartViewer(field.attr("data-field"));
      chartViewers.push({
        chart: chart,
        fields: [{ channel: channel, fieldId: field.attr("data-field") }]
      });
      idx = 0;
      $("#viewers").append(chart.createPanel());
    } else {
      chartViewers[idx]["fields"].push({
        channel: channel,
        fieldId: field.attr("data-field")
      });
    }
    if (!(channel in channelChartMap)) channelChartMap[channel] = [];
...
function updateChartHelper(chart, msg, channel, prefix) {
  ...
  for (var field in msg) {
    ...
    for (var i = 0; i < chart.fields.length; ++i) {
      if (
        chart.fields[i].channel == channel &&
