function channelViewer() {
  ...
  this.createPanel = function(channelClickCB) {
    if (channelClickCB) parent.cb = channelClickCB;

    var wrapper = $("<div />", { class: "channel-viewer" });

    var tableWrapper = $("<div />", { class: "table-responsive" });
    var table = $("<table />", {
      class: "table table-hover table-striped " + "list-table-fixed noselect"
    });
    ...
    panel.ready(function() {
      setTimeout(parent.__proto__.pinPanel, 1);
    });
  ...
  this.handle = function(channel, msg) {
    ...
    if (newChannel) {
      var row = newChannelDiv(
        channel,
        msg.__type,
        0,
        parent.channelIdx[channel],
        parent.cb
      );
  ...
  function newChannelDiv(channel, type, frequency, channelIdx, cb) {
    ...
    row.append(
      $("<td />", { class: "col-md-2 hz " + "hz-" + channelIdx }).text(
        frequency
      )
    );
    return row;
    ...
    setTimeout(function() {
      for (var m in parent.messages) {
        if (parent.messages[m]["utime"] != parent.messages[m]["lastUtime"]) {
          var dt =
            parent.messages[m]["utime"] - parent.messages[m]["lastUtime"];
