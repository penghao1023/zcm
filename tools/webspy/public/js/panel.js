...
function panel() {
  ...
  this.createPanel = function(header, body, panelClass) {
    ...
    if (header) panelHeading.append(header);

    var panelBody = $("<div />", { class: "panel-body" });
    if (body) panelBody.append(body);

    parent.btnClose = $("<div />", {
      class: "btn btn-xs glyphicon " + "glyphicon-remove pull-right",
      stlye: "z-index:99;"
    });
    parent.btnClose.on("click", parent.close);
    parent.btnPin = $("<div />", {
      class: "btn btn-xs glyphicon " + "glyphicon-pushpin pull-right",
      stlye: "z-index:99;"
    });
    parent.btnPin.on("click", function() {
      if ($(this).hasClass("active")) parent.unpinPanel();
      else parent.pinPanel();
    });
    ...
    panelHeading = $("<div />", { class: "panel-heading" }).append(
      panelHeading
    );

    parent.panel.append(panelHeading);
    parent.panel.append(
      $("<div />", { class: "panel-panel-body" }).append(panelBody)
    );
