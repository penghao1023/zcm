function messageViewer(channel) {
  ...
  this.showPanel = function() {
    $("#" + parent.__proto__.panelId + " .content").css(
      "visibility",
      "visible"
    );
  };
  ...
  this.updateViewer = function(msg, utime) {
    for (var f in msg) parent.updateField(parent.c + f, msg[f], utime);
  };
  ...
  this.updateField = function(prefix, field, utime) {
    ...
    switch (type) {
      case "object":
        for (var f in field) parent.updateField(prefix + f, field[f], utime);
        break;
      ...
      case "number":
        if (!(prefix in parent.graphFieldIdx)) {
          var chart = new SmoothieChart({
            ...
            grid: { fillStyle: "#ffffff", strokeStyle: "#ebebeb" },
          ...
          chart.addTimeSeries(line, { lineWidth: 1, strokeStyle: "#000000" });
        ...
        parent.graphs[parent.graphFieldIdx[prefix]]["line"].append(
          new Date().getTime(),
          Number(field)
        );
